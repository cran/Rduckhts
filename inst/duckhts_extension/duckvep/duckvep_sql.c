...
duckvep_register_annotate_relation(duckdb_connection connection)
{
	static const char *const sql[] = {
		...
		"typed AS MATERIALIZED (SELECT *, CASE WHEN __duckvep_alternate IS NOT NULL AND ",
		"regexp_full_match(__duckvep_alternate, '^<(DEL|DUP|TDUP|STR|INV|INS|CNV|UNKNOWN)>$') ",
		"THEN regexp_extract(__duckvep_alternate, ",
		"'^<(DEL|DUP|TDUP|STR|INV|INS|CNV|UNKNOWN)>$', 1) ELSE NULL END ",
		"AS __duckvep_symbolic_structural_type, ",
		"coalesce(__duckvep_explicit_structural_type, ",
		"CASE WHEN __duckvep_alternate IS NOT NULL AND ",
		"regexp_full_match(__duckvep_alternate, '^<(DEL|DUP|TDUP|STR|INV|INS|CNV|UNKNOWN)>$') ",
		"THEN regexp_extract(__duckvep_alternate, ",
		"'^<(DEL|DUP|TDUP|STR|INV|INS|CNV|UNKNOWN)>$', 1) ELSE NULL END) ",
		"AS __duckvep_structural_type FROM source), ",
		"classified AS MATERIALIZED (SELECT *, CASE ",
		"WHEN __duckvep_mate_seq_region IS NOT NULL OR __duckvep_mate_position IS NOT NULL ",
		"THEN 'breakend' WHEN __duckvep_end_position IS NOT NULL ",
		"OR __duckvep_explicit_structural_type IS NOT NULL ",
		...
		"OR contains(__duckvep_alternate, ']'))) THEN 'structural_variant' ",
